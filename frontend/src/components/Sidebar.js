...
const Sidebar = ({
    ...
    const handleStartSimulation = async () => {
        try {
            const res = await axios.post('http://localhost:8000/live-start');
            if (res.data.ok) {
                console.log("Simulation started.");
                onOptimizationStart(optimizationMethod, "Simulating live route...")
            } else {
                console.error("Failed to start simulation:", res.data.message);
            }
        } catch (err) {
            console.error("API error starting simulation:", err);
        }
    };
    ...
    return (
        <div className="bg-slate-900 text-slate-300 h-screen w-96 p-6 overflow-y-auto shadow-lg">
            ...
            {/* Route Form */}
            <RouteForm onOptimize={handleOptimize} onMapLayerChange={onMapLayerChange} />
            ...
            {/* New Start Button for Live Simulation */}
            <div className="mt-4">
                <button
